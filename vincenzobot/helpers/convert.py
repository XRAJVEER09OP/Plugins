...
async def convert_to_image(event, client):
    vincenzo = await event.get_reply_message()
    if not (
            vincenzo.gif
            or vincenzo.audio
            or vincenzo.voice
            or vincenzo.video
            or vincenzo.video_note
            or vincenzo.photo
            or vincenzo.sticker
            or vincenzo.media
    ):
    ...
    if not os.path.exists(downloaded_file_name):
        ...
        return
    if vincenzo and vincenzo.photo:
        vincenzo_final = downloaded_file_name
    elif vincenzo.sticker and vincenzo.sticker.mime_type == "application/x-tgsticker":
        ...
        os.remove(rpath)
        vincenzo_final = image_name20
    elif vincenzo.sticker and vincenzo.sticker.mime_type == "image/webp":
        ...
        if not os.path.exists(imgpath):
            await eor(event, "`Unable To Fetch Shot.`")
            return
        vincenzo_final = imgpath
    elif vincenzo.gif or vincenzo.video or vincenzo.video_note:
        ...
        if not os.path.exists(jpg_file):
            ...
            return
        vincenzo_final = jpg_file
    return vincenzo_final
